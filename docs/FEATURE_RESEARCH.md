...
US) and *real* progress measurement (the reliable-change engine beats the
...
should convert clinical data into *retained clients and collected payments*, not
more notes.
...
- **Gap status:** Genuine gap. You have WhatsApp *sharing* but not scheduled,
  template-based reminders.
- **Build/impact:** Low–medium. The single most quantifiable ROI story ("pays for
  itself in one recovered session/month") and it *extends the existing WhatsApp
  channel*. ⚠️ Must use approved **WhatsApp Business API Utility templates,
...
- **Build/impact:** Medium. Pairs with #1 (booking → confirmation → reminder is one
  flow). Table stakes you currently fail on vs *every* competitor.
...
- **Why:** Not just convenience in India — a **legal dependency.** DPDP makes you a
  ...
  parental consent for minors). Consent-to-record specifically *gates the core
  scribe legally*. AI-scribe recording without consent is driving US class-actions
...
- **Gap status:** Genuine gap.
- **Build/impact:** Medium. Sequence *early, alongside booking* — it's the
...
- **Nuance:** GST only bites above ₹20 lakh/yr turnover; most solo therapists are
  ...
  psychologists may fall *outside* the healthcare GST exemption (18% can apply).
- **Gap status:** Partial — you have Razorpay *subscription* billing (you charging
  the therapist), not therapist→client invoicing.
- **Build/impact:** Low effort, high adoption impact. Strong candidate for *first*
  build.
...
- **Credential nuance (cheap, important):** Capture each therapist's **credential
  ...
  Telemedicine Guidelines apply only to *doctors (RMPs)*, not counsellors (relevant
...
- **Why:** Highest-leverage, lowest-build move. Progress-feedback's biggest payoff
  is in *failing* cases: deterioration drops ~21%→13% and clinically significant
...
- **Why:** The clearest thing competitors do that you don't. Blueprint/Eleos push
  ...
  *and* reduces dropout (~20% of clients drop out). You have one-way artefacts
...
- **Critical caveat:** App engagement is the bottleneck (median completion ~37.6%);
  ...
  WhatsApp channel. Build a *WhatsApp-delivered* check-in loop feeding the
...
- **13. Psychoeducation/worksheet library** (à la Therapist Aid) — a valued
  *stickiness* play with strong market precedent but **no measured outcome
  evidence** — treat as retention, not an efficacy claim.
- **14. Supervision / peer-consultation** — real monthly professional need and an
  income lever, but a *marketplace/matching* feature, not a co-pilot one. Secondary.
...
- **Direct corporate-EAP integration — defer.** The ₹2.6B→₹4B EAP market is real but
  **platform-intermediated** (1to1help, YourDOST); you won't disintermediate them. A
  lighter wedge is *helping solo therapists manage EAP-referred clients*, not
...
1. **Pricing:** The India competitive ceiling is **~₹1,499/mo** (≈ one session) with
   ...
   **modular and unbundled**; never force solo users onto multi-therapist plans to
   reach core features. The AI scribe + MBC justify a *premium above* the generalist
...
2. **Go-to-market:** Borrow **Eleos's playbook of publishing outcome (not
   ...
   uniquely have a *reliable-change engine* — instrument it to produce aggregate
