...
- The frontier is moving from _"document what happened"_ to
  _"help me think and act"_ — live differentials, missed-question
...
_isn't_ a long ambient conversation — the doctor speaks tersely or
dictates.
...
gap / red-flag sidebar that makes them faster _and_ safer in the room.
...
- **Audio is sliced into 30-second windows.**
  ...
  the session ends. (The chunker _does_ accept a `chunkSeconds`
...
- **Rail 2 — live structured note.** A cheap fast model
  ...
  is a _debounced, incremental_ version of today's Pass 2 — not a new
  conceptual pass.
- **Rail 3 — live gaps & flags (the differentiator).** The same
  cadence emits a passive, dismissible sidebar:
  - ❓ _"Not yet asked: duration / fever / drug allergy"_ (completeness
    vs. the specialty template's required HPI/ROS elements)
  - 🔴 _"Chest pain + breathlessness mentioned — consider ECG (ACS
    red flag)"_
  - 💊 _"Ibuprofen + warfarin — interaction"_
  - 🧾 _"Documentation supports ICD-10 X — add Y to avoid undercoding"_
...
2. **Live transport.** The repo's first WebSocket/SSE endpoint.
   **Vercel serverless cannot hold a socket** — the streaming gateway
   ...
   live service _only_ for the streaming gateway. All non-streaming
...
| Rx / orders                             | `prescription.ts` is _therapy-exercise_ recs (not drug Rx)                                                                                             | New `MedicationOrderV1` + `ClinicalOrderV1` + interaction data (§6)                          | Med             |
...
- New prompts + version constants in `packages/llm/src/prompts/index.ts`
  ...
  clinical scribe for an Indian psychotherapy practice"_ — the doctor
...
trend is the _same_ longitudinal-outcome problem the engine already
