...
| **Auth**             | Firebase `__session` cookie (pages) or `Bearer` token (API); outside production, `AUTH_BYPASS` auto-engages without Firebase env → seeded dev fixture. Production ignores bypass and fails closed. Side-effect routes POST-only. | `docs/AUTH_SESSION.md`           |
