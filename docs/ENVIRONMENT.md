...
| `COST_CAP_PER_SESSION_INR` / `COST_CAP_PER_THERAPIST_MONTHLY_INR`                                           | Cost circuit breaker (₹500 / ₹15 000 defaults).                                                                                                         |
| `ASSISTANT_CHAT_PER_MINUTE`                                                                                 | AUD1 — practice-assistant chat rate limit per therapist (default 8/min; 429 over).                                                                      |
| `SHARES_PER_HOUR_CAP` / `MAX_CHUNKS_PER_SESSION`                                                            | AUD1 — patient-share hourly cap per therapist (30) + audio-chunk cap per session (500).                                                                 |
| `RECLAIM_AFTER_MINUTES` / `UNSIGNED_DIGEST_AFTER_HOURS`                                                     | NEXT2/NEXT3 — cron thresholds: stuck-generation reclaim window (30 min) + unsigned-note digest staleness (24 h).                                        |
...
| `LIVE_GATEWAY_MAX_FRAME_BYTES` / `LIVE_GATEWAY_MAX_CONNECTIONS_PER_IP`                                             | AUD2 — WS frame cap (256 KiB default) + per-IP socket cap (10).                                                                                     |
| `LIVE_GATEWAY_TENANT_DAILY_INR_CAP`                                                                                | NEXT4 — per-practitioner daily spend breaker (₹2 000 default, `0` disables); over-cap starts shed as `busy`, resets at IST midnight.                |
