...
> surface**: zero taps to start, everything they _say_ lands on **their**
...
2. **Meds confirm-first; tests/advice auto-land with a 10s undo.** Spoken
   orders parse into "Your plan": a med arrives as a _heard — Confirm_ row
...
3. **Linked evidence everywhere.** Heard rows carry a 🗣 quote-chip that
   ...
   exists). Rationale: Abridge's linked evidence is _the_ trust mechanism
...
1. **Ready screen** — patient + chronic chips + "copilot is watching"
   ...
   _Change_. First tap of the day doubles as the mic-permission gesture.
