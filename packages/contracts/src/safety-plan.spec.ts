...
describe('SafetyPlanV1Schema', () => {
  const valid: SafetyPlanV1 = {
    ...
    helpContacts: [
      { name: 'Friend Arjun', relationship: 'best friend', contact: '+91 9123456789' },
    ],
  ...
  it('requires at least one entry per Stanley & Brown section', () => {
    expect(SafetyPlanV1Schema.safeParse({ ...valid, warningSigns: [] }).success).toBe(false);
    expect(SafetyPlanV1Schema.safeParse({ ...valid, internalCoping: [] }).success).toBe(false);
    expect(SafetyPlanV1Schema.safeParse({ ...valid, socialDistractions: [] }).success).toBe(false);
  ...
  it('caps each section at 8 entries', () => {
    ...
    expect(SafetyPlanV1Schema.safeParse({ ...valid, warningSigns: tooMany }).success).toBe(false);
  });
});
