...
| `docs/CARE_GROWTH_SYSTEM.md`       | **Growth system** — Care attraction/attachment design: onboarding arc, assessment aha, habit engine, monetization (pricing CLOSED), growth loops, ethics charter, sprints CG1–CG6                                                                                                                       |
| `docs/CARE_PSYCHOLOGIST.md`        | **The Proper Psychologist (CP1–CP8) — PLANNED**: kill the auto-wrap (clock authority + resumable transport), live structure engine (six tools + phase rail), measured baseline + graded risk ladder, 5-Ps formulation, manualized arcs + toolkit, document-grade reports, MBC every session, wire proof |
...
| Work on in-app video sessions (Cureocity Sessions) | `apps/web/lib/livekit.ts` (env gate + tokens) + `components/video/VideoSessionRoom.tsx` + `/app/video/[appointmentId]` + `/p/appointments/[id]/join` — LiveKit Cloud; OFF (Meet/Zoom-link fallback) until `LIVEKIT_URL/API_KEY/API_SECRET` are set |
