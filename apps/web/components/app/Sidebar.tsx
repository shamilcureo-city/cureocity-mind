...
interface SidebarProps {
  ...
  /// PC2 — surface the super-admin console link (ADMIN role only).
  isAdmin?: boolean;
}

export function Sidebar({ usage = null, vertical = 'THERAPIST', isAdmin = false }: SidebarProps) {
            ...
            <ul className="mt-1 space-y-1">
              <li>
                <Link
                  ...
                  aria-current={path.startsWith('/app/admin') ? 'page' : undefined}
                >
                  <svg
                    aria-hidden
                    width="18"
