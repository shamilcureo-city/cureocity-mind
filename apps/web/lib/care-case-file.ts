...
export function languageGuidance(preferred: string, spoken: string[]): string {
  ...
  return `Speak with them in ${mixName} — the everyday, code-mixed way people actually talk, NOT pure formal ${primaryName} and NOT pure English. Your ${primaryName} must be the SPOKEN, everyday register — how people talk at home or with a close friend — never textbook, literary, or news-reader ${primaryName}: use the simple common words, short sentences, natural rhythm. If a ${primaryName} word would sound bookish or you are not sure it is the word people actually SAY, use the everyday English word instead — that is exactly how real ${mixName} sounds, and stiff ${primaryName} is worse than English. Code-mixing means blending WITHIN one sentence; say every thought exactly ONCE. NEVER say something in English and then repeat it in ${primaryName} (or the reverse) — you are not a translator, and saying the same thing twice in two languages sounds robotic and wastes the session. Pronounce carefully and unhurriedly. Mirror the user — if they lean more English or more ${primaryName}, follow them.`;
}
